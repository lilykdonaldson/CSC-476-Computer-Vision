...
def plot_harris_points(image,filtered_coords):
    ...
    plt.show()
    plt.imshow(image,cmap='gray')
    plt.plot([p[1]for p in filtered_coords], [p[0] for p in filtered_coords],'d')
    plt.show()
    plt.imshow(image,cmap='gray')
    plt.plot([p[1]for p in filtered_coords], [p[0] for p in filtered_coords],'p')
    plt.show()


# calling the harris corner detector
im = misc.imread('classroom.jpg',flatten=1)
