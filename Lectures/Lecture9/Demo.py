...
first = cv2.warpAffine(img,M,(cols,rows))

#put it back
M = cv2.getRotationMatrix2D((cols/2,rows/2),45,1)

second = cv2.warpAffine(first,M,(cols,rows))

plt.subplot(121),plt.imshow(first),plt.title('Input')
plt.subplot(122),plt.imshow(second),plt.title('Output')
plt.show()